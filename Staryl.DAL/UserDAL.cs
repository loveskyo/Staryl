...
public int Create(UserInfo model)
        {         Database db = DBHelper.CreateDataBase();
         ...
         sb.Append("Email,Gender,Mobile,UserType,RecommendUser,RealName,Birthday,Avatar,CreateDate,CreateIP,IsVIP,Password,Status,IsLogin");
         sb.Append(") values(");
         sb.Append("@Email,@Gender,@Mobile,@UserType,@RecommendUser,@RealName,@Birthday,@Avatar,@CreateDate,@CreateIP,@IsVIP,@Password,@Status,@IsLogin);SELECT @@IDENTITY;");
      ...
      { 
         ...
         sb.Append("Email=@Email,Gender=@Gender,Mobile=@Mobile,UserType=@UserType,RecommendUser=@RecommendUser,RealName=@RealName,Birthday=@Birthday,Avatar=@Avatar,CreateDate=@CreateDate,CreateIP=@CreateIP,IsVIP=@IsVIP,Password=@Password,Status=@Status,IsLogin=@IsLogin");
...
bool suc = BaseDAL.ExecuteTransactionScopeInsert(this.ToDataTable(list), 250, "User"); return suc; }
        ...
        { 
            ...
            if (list.Count > 0)
            {
                ...
                string columnNameList = "?Id??Email??Gender??Mobile??UserType??RecommendUser??RealName??Birthday??Avatar??CreateDate??CreateIP??IsVIP??Password??Status??IsLogin?"; 
